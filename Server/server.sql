...
drop table Car1 CASCADE CONSTRAINTS;
drop table Price CASCADE CONSTRAINTS;
drop table Price1 CASCADE CONSTRAINTS;
...
insert into Car values (5, 'Chevrolet', 5, 'Mini', 4, 'No'); 
insert into Car values (6, 'Ford', 2, 'Large', 7, 'Yes');
insert into Car values (7, 'Ford', 4, 'Mini', 4, 'Yes'); 
insert into Car values (8, 'Hyundai', 10, 'Large', 8, 'No');
...
insert into Car values (12, 'Mercedes-Benz', 2, 'Mini', 4, 'No'); 
...
create table Car1(carNo number, carName varchar2(40), totalCar number, carType varchar2(20), totalSit number, available varchar2(10), PRIMARY KEY(carNo));
BEGIN

	FOR i IN (SELECT * FROM Car WHERE carType = 'Large')
	LOOP
		insert into Car1 VALUES (i.carNo, i.carName, i.totalCar, i.carType, i.totalSit, i.available);
	END LOOP;

END;
/

create table Price1(priceNo number, carType varchar2(20), timesP number, locationNo number, totalPrice number, PRIMARY KEY(priceNo), FOREIGN KEY(locationNo) REFERENCES Location(locationNo));
BEGIN

	FOR j IN (SELECT * FROM Price WHERE carType = 'Large')
	LOOP
		insert into Price1 VALUES (j.priceNo, j.carType, j.timesP, j.locationNo, j.totalPrice);
	END LOOP;
...
CREATE OR REPLACE PACKAGE Car_rental AS

	FUNCTION myF(C1 Car.carType%TYPE, L1 IN Location.location1%TYPE, L2 in Location.location1%TYPE, T1 in number)
	RETURN number;
	
END Car_rental;
/

CREATE OR REPLACE PACKAGE BODY Car_rental AS
	...
	BEGIN
		IF C1 = 'Large' THEN
			select Price1.totalPrice into T2 from Price1 inner join Location on Price1.locationNo = Location.locationNo where Location.location1 = L1 and Location.location2 = L2;
			T2 := T2 * T1;
			DBMS_OUTPUT.PUT_LINE('Total price : ' || T2);	
		ELSE
			select system.Car_rental2.myF2@site(C1, L1, L2, T1) into T2 from dual;
			T2 := T2 * T1;
			DBMS_OUTPUT.PUT_LINE('Total price : ' || T2);
		END IF;
...
CREATE OR REPLACE PROCEDURE carP(pro IN Car1.carType%TYPE)
IS
BEGIN
    
	IF pro = 'Large' THEN
		For K in (SELECT carNo, carName from Car1 where carType = pro and available = 'Yes')
		Loop
	...
	ELSE
		...
		For K in (SELECT carNo, carName from Car2@site where carType = pro and available = 'Yes')
		Loop
		DBMS_OUTPUT.PUT_LINE(K.carNo ||' ' || K.carName);
		End Loop;
...
select l.locationNo, l.location1, l.location2, l.distanceKm from Location l;
select * from myview;


ACCEPT U char PROMPT "Enter car type (Large/Mini) = "
ACCEPT X char PROMPT "Enter loction 1 = "
ACCEPT Y char PROMPT "Enter loction 2 = "
ACCEPT Z number PROMPT "Enter duration = "
